...
const dataALLPath = "./Group/UBU/dataALL.json";

if (!fs.existsSync(dataALLPath)) {
  console.error(`File ${dataALLPath} does not exist`);
  exec(
    `cd ${path.dirname(__filename)} && python3 ./main.py`,
    (error, stdout, stderr) => {
      if (error) {
    ...
    try {
      ...
      result.map((data) => {
        if (!mergedObjects.map((mo) => mo.code).includes(data.code)) {
          mergedObjects.push({
            ...data,
            sec_count: filteredData.filter((df) => df.code === data.code)
              .length,
          });
        }
      });
      ...
      fs.readFile(
        ...
        (err, data) => {
          ...
          const searchResults = dataALL.filter((item) => {
            return (
              (searchData.type.includes(item.type) ||
              ...
              (item.time
                .split(" & ")
                .filter((fitem) => fitem.substring(2, 4) === searchData.time)
                .length > 0 ||
  ...
  const scheduledFunction = () => {
    ...
    if (seconds > 0) {
      process.stdout.write(
        `\x1b[K\x1b[90mRequested done on\x1b[0m ${cache_updated} \x1b[90m(${count}) \x1b[33m| \x1b[37m${seconds}\x1b[33m's left...\r`
      );
      seconds--;
      return;
    } else if (seconds == 0) {
      ...
      if (cache_updated === "none") {
        process.stdout.write(
          `\x1b[K\x1b[90mFirst Running \x1b[33m| \x1b[32mUpdating...\r`
        );
      } else {
        process.stdout.write(
          `\x1b[K\x1b[90mRequested done on\x1b[0m ${cache_updated} \x1b[33m| \x1b[90m${count} \x1b[33m| \x1b[32mUpdating...\r`
        );
      }
